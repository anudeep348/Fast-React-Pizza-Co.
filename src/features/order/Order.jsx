...
function Order() {
  ...
  const priorityPriceDerived = Math.round(orderPrice * 0.2);

  const deliveryIn = calcMinutesLeft(estimatedDelivery);

  return (
    <div className="space-y-8 px-4 py-6">
      <div className="flex-weap flex items-center justify-between gap-2">
      ...
      <div className="space-y-23 bg-stone-200 px-6 py-5 ">
        ...
        {priority && (
          <p className="text-sm font-medium text-stone-600">
            Price priority: {formatCurrency(priorityPriceDerived)}
          </p>
        )}
        ...
        <p className="font-bold">
          To pay on delivery:{" "}
          {formatCurrency(
            priority ? orderPrice + priorityPriceDerived : orderPrice,
          )}
        </p>
      </div>
